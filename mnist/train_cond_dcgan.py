...
sys.setrecursionlimit(50000) # workaround for pickle recursion error
sys.path.append('..')
...
compiled_fns_filename = model_dir + '/compiled_functions'
if not os.path.exists(compiled_fns_filename):
    print('COMPILING')
    t = time()
    ...
    compiled_fns_file = open(compiled_fns_filename, 'wb')
    pickle.dump((_train_g, _train_d, _gen), compiled_fns_file)
else:
    print('compiled functions file found, loading...')
    try:
        compiled_fns_file = open(compiled_fns_filename, 'rb')
        _train_g, _train_d, _gen = pickle.load(compiled_fns_file)
    except Exception:
        print('error loading datafile, deleting file and exiting.')
        os.remove(compiled_fns_filename)
        exit(1)


tr_idxs = np.arange(len(trX))
trX_vis = np.asarray([[trX[i] for i in py_rng.sample(list(tr_idxs[trY == y]), 20)] for y in range(10)]).reshape(200, -1)
...
def gen_samples(n, nbatch=128):
    ...
    labels = []
    n_gen = 0
    for i in range(math.floor(n / nbatch)):
...
for epoch in range(1, niter + niter_decay + 1):
    ...
    if (epoch - 1) % 5 == 0:
        ...
        log = [n_epochs, n_updates, n_examples, time() - t, va_nnc_acc_1k, va_nnc_acc_10k, va_nnc_acc_100k, va_nnd_1k,
               va_nnd_10k, va_nnd_100k, g_cost, d_cost]
        print('%.0f %.2f %.2f %.2f %.4f %.4f %.4f %.4f %.4f' % (
        epoch, va_nnc_acc_1k, va_nnc_acc_10k, va_nnc_acc_100k, va_nnd_1k, va_nnd_10k, va_nnd_100k, g_cost, d_cost))
        f_log.write(bytes(json.dumps(dict(zip(log_fields, log))) + '\n', 'UTF-8'))
        f_log.flush()
